...
For the terminal emulation run PuTTY (or anyother application supporting telnet) and configure a telnet comunication with host name:localhost and port:3456 

If when performing step 5 Renode returns some error concerning the sensors, probably the recompiled version (supporting BMP180) must be installed.
In this case install the recompiled version using "renode_1.11.0.msi" in the curret folder and re-run all the steps 1-7.